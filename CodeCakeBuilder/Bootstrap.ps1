...
if (Get-Command $msbuildExe -ErrorAction SilentlyContinue) {
    ...
    Install-PackageProvider -Name NuGet -Scope CurrentUser | Out-Null
    ...
    Install-Script -Name Resolve-MSBuild -Scope CurrentUser
...
# Download docFx
$docfxZipPath = Join-Path $toolsDir "docfx.zip"
$docfxPath = Join-Path $toolsDir "docfx"
if (!(Test-Path $docfxPath)) {
    Invoke-WebRequest -Uri https://github.com/dotnet/docfx/releases/download/v2.22/docfx.zip -OutFile $docfxZipPath
    Add-Type -AssemblyName System.IO.Compression.FileSystem
    [IO.Compression.ZipFile]::ExtractToDirectory($docfxZipPath, $docfxPath)
}

# Download docfx MSDN ref
$msdnRef = [IO.Path]::Combine($solutionDir, "Documentation", "update-msdn.ps1");
Write-Information $msdnRef
Invoke-Expression $msdnRef
